...
class SuccessPage extends Component{
    constructor(props){
        ...
        this.state = {
            hostels1 : [],
            hostels2:[] , 
            hname : this.props.match.params.name
        }
    }
    componentDidMount= () =>{
   ...
   render(){
      ...
      let book = this.state.hostels2.filter((host)=> host.hostelName === this.state.hname);
      console.log(data[0]?.hostelName);
      console.log(book[0]?.hostelName);
       return(<div>
...
<div class="card" style={{width: "24rem", height :"25rem",  marginLeft : "500px" }}>
<img class="card-img-top" src={data[0]?.imgUrl} alt="Card image cap"/>
  <div class="card-body">
       <h6 class="card-subtitle mb-2 text-muted">{book?.userName}</h6>
       <h5 class="card-title">{data[0]?.hostelName}</h5>
       <p class="card-text">{data[0]?.contactName}</p>
       <p class="card-text">{book[0]?.rent}</p>
